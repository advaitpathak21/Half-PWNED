...
- going to the page, we get an error and then when I refreshed it, I got another error showing the stack trace to the code that was throwing the error
    - ![alt text](/docs/blog/attachments/agile.png)
...
We also need the appâ€™s name, which is oftentimes Flask. But other possible values are DebuggedApplication and wsgi_app. So again, some trial and error might be required if the usual values donâ€™t generate a valid PIN.
```
